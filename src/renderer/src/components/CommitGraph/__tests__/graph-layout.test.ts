...
// date-order rows GitKraken displays. Each entry is "hash parent[ parent...]".
// The expected lanes were read directly off a GitKraken screenshot of this repo.
...
// Expected lanes for each row. Matches GitKraken's lane disposition, except the
...
describe('computeGraphLayout', () => {
  ...
  it('matches GitKraken lane assignment on the demo repo', () => {
    const layout = computeGraphLayout(buildCommits(TOPO))
    expect(layout.map(c => c.lane)).toEqual(GK_LANES)
  })
