...
export function useSettingsPage({ onClose, showToast, onUpdateFound, embedded = false, onBrowseThemes }: SettingsModalProps) {
  ...
  /** Providers whose list came from the catalog, so whose key was never checked. */
  const [unverifiedModels, setUnverifiedModels] = useState<Record<string, boolean>>({})
  ...
  const fetchModels = async (provider: AIProvider, key: string, baseUrl?: string) => {
    ...
    // Declared by the catalog rather than answered: the models are real, the
    // key is unchecked, and the row says which of the two it knows.
    setUnverifiedModels(prev => ({ ...prev, [provider]: !!r.unverified }))
