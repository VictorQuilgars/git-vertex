...
export function useSidebar(props: SidebarProps) {
  ...
  home, mergeTarget, launchpad, onOpenActivityTab,
} = props
  ...
  const [activity, setActivity] = useState<{ points: { at: number; author: string; hash: string }[]; truncated: boolean } | null>(null)
  ...
  useEffect(() => {
    ...
    // The chart's points: one log per reload, capped by the core.
    if (show('overview')) window.gitAPI.getActivity?.().then(r => setActivity(r ?? null)).catch(() => setActivity(null))
