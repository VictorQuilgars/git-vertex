...
import { readSettings, writeSettings, resealSettings } from './settings-store'
...
/**
 * What an older version left readable, sealed before anything else runs — and
 * remembered, so the window can say so.
 *
 * Sealing on write was correct and was not enough: nothing forced a write, so
 * four API keys stayed in clear for six releases. This is the half that had to
 * be added, and it is only half of THIS: a file rewritten now says nothing
 * about the copies a backup or a synced folder took while it was readable, so
 * the keys it fixes still have to be regenerated by the person who owns them.
 * `aiKeysExposed` carries their provider labels to the notification.
 */
function sealWhatWasLeftInClear(): void {
  const fixed = resealSettings()
  if (!fixed.length) return
  const labels = [...new Set(fixed.map(key => {
    const def = AI_PROVIDER_CATALOG.find(p => p.keySetting === key)
    if (def) return def.label
    // The two legacy spellings and the forge tokens, which no catalog names.
    return { groqApiKey: 'Groq', geminiApiKey: 'Google (Gemini)', githubToken: 'GitHub',
      githubEnterpriseToken: 'GitHub Enterprise' }[key] ?? key
  }))]
  console.log(`[settings] sealed ${fixed.length} credential(s) left in clear: ${labels.join(', ')}`)
  const s = readSettings()
  // Appended, never replaced: a second launch that finds nothing must not
  // erase a warning the user has not acted on yet.
  const already = (s.aiKeysExposed ?? '').split('\n').filter(Boolean)
  const merged = [...new Set([...already, ...labels])]
  if (merged.length !== already.length) { s.aiKeysExposed = merged.join('\n'); writeSettings(s) }
}

app.whenReady().then(() => {
  electronApp.setAppUserModelId('com.victor.gitvertex')
  sealWhatWasLeftInClear()
