...
### Changed
- **A section header shows its count, or its controls — never both.** The `+` and the refresh appeared on hover but sat beside the count, so the corner carried two things competing for the same glance. They swap now: the count fades as the controls arrive, and back when the pointer leaves. Both stay in the tree throughout, so Tab still reaches the buttons, and a refresh already in flight holds its place whatever the pointer is doing. On a touch screen, where there is no hover to wait for, everything is simply shown.
- **The drawer is wider, and it arrives.** It was as wide as the form's minimum; it now takes the room a query and its reference need to be read rather than wrapped, still clamped to what is left of the window. And it slides out of the panel's edge instead of appearing — briefly, and not at all for anyone who has asked not to be moved.
