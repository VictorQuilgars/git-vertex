...
// Absolute path of the real git dir — handles worktrees/submodules where
// `.git` is a file pointing elsewhere.
export function getGitDir(repoRoot: string): string | null {
  try {
    return String(execSync('git rev-parse --absolute-git-dir', GIT_OPTS(repoRoot))).trim()
  } catch {
    return null
  }
}
