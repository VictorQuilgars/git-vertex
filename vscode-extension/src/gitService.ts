...
function conflictKind(index: string, workingDir: string): ConflictKind {
...
export class GitService {
  ...
  /**
   * The same contract for the commands whose NON-ZERO EXIT is the answer:
   * `merge-tree` exits 1 to mean "would conflict" and puts the paths on stdout,
   * which `run` above would reject on and throw away.
   */
  ...
  // Raw git access for host-side helpers (AI prompts need diffs/logs in
  // formats no dedicated method produces). NOT part of the webview surface
  // conceptually, though the reflective bridge technically exposes it — the
  // webview is trusted, first-party code.
  raw(args: string[]): Promise<string> {
    return this.git.raw(args)
  }

  // Human-readable labels for the two sides of the current conflict (same
  // logic as the desktop's git-service). During a rebase HEAD is the NEW
  // BASE (detached) and the replayed commit is the branch being rebased.
  ...
  async getLog(options: { maxCount?: number; all?: boolean; refs?: string[]; excludes?: string[] } = {}): Promise<{ commits: CommitNode[] }> {
    ...
    return { commits: await core.log(this.run, { ...(await this.refScope(options)), maxCount: options.maxCount ?? 300 }) }
  }
  ...
  private async detachedHeadLabel(): Promise<string | null> {
    ...
    for (const dir of ['rebase-merge', 'rebase-apply']) {
      try {
        ...
        const ref = readFileSync(pathResolve(this.repoPath, rel), 'utf8').trim()
  ...
  // One implementation, in git-core, which the desktop runs too; this is the
  // adapter. `runRaw` rather than `run` because merge-tree answers by its exit
  // code — the core says why.
  ...
  // Kept in step with the desktop implementation (src/main/git-service.ts): same
  // source (`status`), same filter, same shape. This used to ask a different
  // question — `diff --name-only --diff-filter=U` — which returns the same paths
  // but carries no XY codes, so it could not tell a modify/delete from a content
  // conflict.
  ...
  // ── Rebase state (drives the "rebase in progress" editor tab) ───

  // Resolve the real git dir — `.git` is a *file* ("gitdir: <path>") in
  ...
  private gitDirPath(): string {
    ...
    try {
      if (fs.statSync(dotGit).isFile()) {
        ...
        if (m) return path.resolve(this.repoPath, m[1])
      }
    } catch { /* not a repo yet — existsSync checks below all fail */ }
    return dotGit
  }
  ...
  private async attachRebaseDates(...stepLists: RebaseStep[][]): Promise<void> {
    ...
    try {
      const out = await this.git.raw(['log', '--no-walk', '--date=relative', '--format=%H%x01%ad', ...hashes])
      const dates = new Map<string, string>()
      for (const line of out.trim().split('\n')) {
        const [hash, date] = line.split('\x01')
        if (hash) dates.set(hash, date ?? '')
      }
      for (const list of stepLists) for (const s of list) if (s.hash) s.date = dates.get(s.hash)
    } catch { /* best effort — rows just show without a date */ }
  }

  async getRebaseState(): Promise<RebaseState> {
    const fs = require('fs') as typeof import('fs')
    const path = require('path') as typeof import('path')
    const g = this.gitDirPath()
    const read = (...seg: string[]): string | null => {
      try { return fs.readFileSync(path.join(g, ...seg), 'utf8') } catch { return null }
    }
    // Todo-format lines: "<action> <sha> <subject>" plus hash-less directives
    // (exec, break, label, reset, merge, noop). Comments and blanks skipped.
    // Recent git writes the subject as a comment ("pick <sha> # <subject>").
    const parseSteps = (text: string | null): RebaseStep[] => {
      const steps: RebaseStep[] = []
      for (const line of (text ?? '').split('\n')) {
        const t = line.trim()
        if (!t || t.startsWith('#')) continue
        const m = t.match(/^([a-z]+)\s+([0-9a-f]{7,40})(?:\s+(.*))?$/i)
        if (m) {
          const subject = (m[3] ?? '').replace(/^#\s*/, '')
          steps.push({ action: m[1].toLowerCase(), hash: m[2], shortHash: m[2].slice(0, 7), subject })
        } else {
          const mm = t.match(/^([a-z]+)(?:\s+(.*))?$/i)
          if (mm) steps.push({ action: mm[1].toLowerCase(), hash: '', shortHash: '', subject: mm[2] ?? '' })
        }
      }
      return steps
    }
    const mergeDir = path.join(g, 'rebase-merge')
    if (fs.existsSync(mergeDir)) {
      const done = parseSteps(read('rebase-merge', 'done'))
      const todo = parseSteps(read('rebase-merge', 'git-rebase-todo'))
      const onto = (read('rebase-merge', 'onto') ?? '').trim()
      const headName = (read('rebase-merge', 'head-name') ?? '').trim().replace(/^refs\/heads\//, '')
      const msgnum = parseInt((read('rebase-merge', 'msgnum') ?? '').trim(), 10)
      const end = parseInt((read('rebase-merge', 'end') ?? '').trim(), 10)
      await this.attachRebaseDates(done, todo)
      return {
        inProgress: true,
        interactive: read('rebase-merge', 'interactive') !== null,
        headName,
        ontoHash: onto,
        ontoShort: onto.slice(0, 7),
        stepCurrent: Number.isFinite(msgnum) ? msgnum : done.length,
        stepTotal: Number.isFinite(end) ? end : done.length + todo.length,
        done,
        todo,
        stoppedSha: (read('rebase-merge', 'stopped-sha') ?? '').trim() || null,
        conflicts: (await this.getConflictedFiles()).files,
      }
    }
    const applyDir = path.join(g, 'rebase-apply')
    if (fs.existsSync(applyDir)) {
      // am-based backend (plain `git rebase` on older configs): no todo file,
      // only step counters.
      const next = parseInt((read('rebase-apply', 'next') ?? '').trim(), 10)
      const last = parseInt((read('rebase-apply', 'last') ?? '').trim(), 10)
      const onto = (read('rebase-apply', 'onto') ?? '').trim()
      return {
        inProgress: true,
        interactive: false,
        headName: (read('rebase-apply', 'head-name') ?? '').trim().replace(/^refs\/heads\//, ''),
        ontoHash: onto,
        ontoShort: onto.slice(0, 7),
        stepCurrent: Number.isFinite(next) ? next : 0,
        stepTotal: Number.isFinite(last) ? last : 0,
        done: [],
        todo: [],
        stoppedSha: (read('rebase-apply', 'original-commit') ?? '').trim() || null,
        conflicts: (await this.getConflictedFiles()).files,
      }
    }
    return {
      inProgress: false, interactive: false, headName: '', ontoHash: '', ontoShort: '',
      stepCurrent: 0, stepTotal: 0, done: [], todo: [], stoppedSha: null, conflicts: [],
    }
  }
  ...
  /**
   * Drop several commits in ONE rebase (#69) — the desktop's twin. A loop of
   * single drops would not survive its first step: every drop rewrites the
   * hashes above it. One sequence from the oldest selection's parent, every
   * selected line marked drop.
   */
  ...
  private buildMessageEditor(messages?: string[]): { gitEditor: string; cleanup: () => void } {
    ...
    const tmpDir = os.tmpdir()
    const msgDir = path.join(tmpDir, `gitvertex-msgs-${runId}`)
    const counterFile = path.join(tmpDir, `gitvertex-msg-counter-${runId}.txt`)
    const scriptFile = path.join(tmpDir, `gitvertex-msgeditor-${runId}.sh`)
...
`, 'utf8')
  ...
  async skipRebase(): Promise<{ success: boolean; error?: string; conflict?: boolean }> {
    try {
      await this.gitExec(['rebase', '--skip'])
      if (this.rebaseInProgress() && await this.hasUnmergedPaths()) {
  ...
  // Rewrite the remaining steps of a paused interactive rebase (action changes
  // and/or reordering from the rebase tab), so the next --continue picks up
  // the edited plan. Only touches rebase-merge/git-rebase-todo — steps already
  // in `done` are baked into history and can't be edited retroactively.
  async updateRebaseTodo(steps: { action: string; hash: string }[]): Promise<{ success: boolean; error?: string }> {
    const fs = require('fs') as typeof import('fs')
    const path = require('path') as typeof import('path')
  ...
  async continueCherryPick(): Promise<{ success: boolean; error?: string }> {
    try { await this.gitExec(['cherry-pick', '--continue']); return { success: true } }
  ...
  async continueRevert(): Promise<{ success: boolean; error?: string }> {
    try { await this.gitExec(['revert', '--continue']); return { success: true } }
    catch (e: any) { return { success: false, error: e.stderr ?? e.message } }
  }
  ...
  // rebase planner) instead of silently aborting it — the extension's
  // rebase-detection watcher then auto-opens the "Rebase in progress" tab so
  // the user can resolve conflicts and continue, rather than losing the
