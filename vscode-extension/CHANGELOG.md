...
- **The pull requests and the issues are in the left panel**, as two sections among the branches and the tags, with their counts. Each row says what the forge said: a state icon (a draft is greyed), the number and the title, then the author, the age and the comment count. Right-click an issue to start its branch — the same gesture, and the same single row component, as the GitHub tab, so the two lists can no longer drift apart. Rest on a row and a **hover card opens over the graph**: the description rendered as markdown (headings, task lists, code), the status, the full labels, assignees and reporter — clamped to the window and never scrolling: the card is a preview, what does not fit fades out, and clicking the card opens the issue itself. The sections start folded: the graph is the point of the window, and the counts on their headers already say what is behind. Inside, each section is **named groups**, not one flat list: *My Pull Requests*, *Assigned To Me*, *Awaiting My Review* and *All Pull Requests* for the first — the three account groups exist only when someone is signed in, and an empty one still shows its 0, which is what says the query ran — and *All Open Issues* for the second. The asymmetry is the point: a pull request is something you are personally on the hook for, an issue is not. Above the groups, each section carries a **repository selector** — the remote's repository by default, any other one a pick away, and the seam where another provider would eventually be chosen — and a **search box**: a display lens like the staging filter, narrowing what is already shown without re-querying, the counts still counting everything. In the VS Code panel they are two more views on the activity rail, which is how that rail already works. A repository with no GitHub — or nothing to authenticate with — shows no section at all, rather than two empty ones.
