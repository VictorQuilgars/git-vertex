...
### Added
- **The branches and tags are under the commit message, not in a column.** The BRANCH / TAG column was 164 pixels that every row paid for, while only a handful of commits carry a ref at all — a sixth of the panel's width spent on nothing. They now hang under the message they belong to, as one chip carrying the branch, the remote it is published on and the issue its branch is working on; everything but the first is an icon until you hover it. The commit graph moves to the far left, where it starts. The desktop app keeps its column, which is what a window with room to spare should do.
