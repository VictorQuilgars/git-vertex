...
- **Disconnecting says what it actually did.** Your GitHub session belongs to VS Code, not to us: forgetting our token does not sign you out of it. The panel now says so and points at the Accounts menu, rather than clearing the field and leaving you authenticated with no explanation. Settings also states where the account came from — *Signed in through VS Code* or a token we hold — because the two behave differently when you disconnect.
