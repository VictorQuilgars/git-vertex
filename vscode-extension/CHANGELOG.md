...
- **A clean tree's Working Changes pane says what comes next instead of nothing.** The branch header stays, and under it: *Next steps* — publish this branch if it has no upstream, push N if it is ahead, pull N if it is behind — and *Start new* with *Create a branch…*. Every row is true of the repository or it is not drawn: no *Publish* on a published branch, no *Push 3* on a branch level with its upstream.
