...
### Added
- **The branches and tags can sit under the commit message instead of in their own column.** Right-click the graph header → *Branches under the message*. The BRANCH / TAG column is 164 pixels wide and every row pays for it, while only a handful of commits carry a ref at all — in a narrow window, or the VS Code panel, that is a sixth of the width spent on nothing. Under the message, the space is used only by the rows that have something to show, and a row grows a line only when it does. Off by default: it is a trade, not an improvement — the column sorts and scans better when there is room for it.
