...
- **The pull requests and the issues are in the left panel**, as two sections among the branches and the tags, with their counts. Each row says what the forge said: a state icon (a draft is greyed), the number and the title, the labels as coloured dots with their names a tooltip away, then the author, the age and the comment count. Right-click an issue to start its branch — the same gesture, and the same single row component, as the GitHub tab, so the two lists can no longer drift apart. Rest on a row and a **hover card opens over the graph**: the description rendered as markdown (headings, task lists, code), the status, the full labels, assignees and reporter — clamped to the window, scrolling inside itself, which is what lets it exist in a 400-pixel panel. The sections start folded: the graph is the point of the window, and the counts on their headers already say what is behind. In the VS Code panel they are two more views on the activity rail, which is how that rail already works. A repository with no GitHub — or nothing to authenticate with — shows no section at all, rather than two empty ones.
