...
### Changed
- **The git icons read at the size they are actually drawn.** A branch, a merge, a rebase, a pull request, a worktree — the nodes in each were open rings, and an open ring of that radius has a **0.12-pixel hole** at the eleven pixels the sidebar draws them at. The ring closed anyway, so it was never a ring; it was a soft blob with a fuzzy edge, which is why the marks looked thin and hard to tell apart. The nodes are filled discs now, and slightly larger. The set loses one distinction — an arrival point used to be filled where a source was open — but that distinction was invisible below roughly 24 pixels, which is where none of these are drawn.
